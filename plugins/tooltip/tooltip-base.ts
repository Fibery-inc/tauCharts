...
export default class Tooltip {
  ...
  _initDomEvents() {
    ...
    if (this.settings.clickable) {
      this._outerClickHandler = (e) => {
        const clickableItems = Array.from(document.querySelectorAll(`.${this.settings.clsClickable}`)).concat(
          this.getDomNode()
        );
  ...
  _showTooltip(data, cursor) {
    ...
    this._tooltip.content(content).position(cursor.x, cursor.y).place(settings.align).show().updateSize();
  ...
  getFieldFormat(k) {
    const format = this._formatters[k] ? this._formatters[k].format : (x) => String(x);
    return this.settings.escapeHtml ? (x) => utils.escapeHtml(format(x)) : format;
  }
