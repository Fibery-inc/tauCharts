...
import { Plot, ScaleConfig, ScaleFields } from "../src/definitions";
...
const toggleIconTemplate = () =>
  [`<span class="${CLS_VALUE_TOGGLE}">`, `<span class="${CLS_VALUE_TOGGLE}__icon"></span>`, "</span>"].join("");
...
class CategoryFilter {
  ...
  _getCategoriesInfo() {
    ...
    const categories = scales.map((scale) => {
      ...
      const dataSource = this._chart.getDataSources({
        excludeFilter: ["category-filter"],
      });
  ...
  _toggleCategory(key, type: "toggle" | "focus") {
    ...
    const nodes = Array.from(this._node.querySelectorAll(`.${CLS_VALUE}`)).reduce((map, node) => {
      const k = node.getAttribute("data-key");
      map[k] = node as HTMLElement;
      return map;
    }, {} as { [key: string]: HTMLElement });
    const items = Object.keys(this._filterKeys).map((k) => {
    ...
    this._lastClickedScrollInfo = {
      key,
      top: target.node.getBoundingClientRect().top - this._getScrollContainer().getBoundingClientRect().top,
    };
    this._chart.refresh();
  }
  ...
  _toggleHighlight(key: string, highlight: boolean) {
    ...
    const matches = highlight ? createRowPredicate(dim, value) : (row) => null;

    this._chart.select((unit) => true).forEach((unit) => unit.fire("highlight", matches));
  }
