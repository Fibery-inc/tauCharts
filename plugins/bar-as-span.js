    ...
    onSpecReady: function (chart, specRef) {
      chart.traverseSpec(specRef, function (unit, parentUnit) {
        if (unit.type === "ELEMENT.INTERVAL") {
          unit.transformModel = [unit.flip ? transformX0 : transformY0, transformMultiline];

          unit.adjustScales = [adjustValueScale];

          unit.guide.enableColorToBarPosition = false;

          unit.guide.label = unit.guide.label || {};
          unit.guide.label.position =
            unit.guide.label.position ||
            (unit.flip
              ? ["inside-start-then-outside-end-horizontal", "hide-by-label-height-horizontal"]
