  ...
  "ELEMENT.BOX-WHISKERS",
  {
    ...
    drawElement: function (container, tuples) {
      ...
      var drawPart = function (that, props) {
        ...
        var part = that.selectAll("." + props.class).data(function (row) {
          return [row];
        }, screenModel.id);
        part.exit().call(
          createUpdateFunc(speed, null, { width: 0 }, function (node) {
      ...
      var drawElement = function (selection) {
        drawPart(
          selection,
          self.buildRangeModel(screenModel, {
            size: 2,
            y0: MIN,
            y1: Q1,
            class: "range-min-Q1",
      ...
      var frameGroups = container.selectAll(".box-whiskers-node").data(tuples);
      frameGroups.exit().remove();
      frameGroups.call(drawElement);
      frameGroups.enter().append("g").attr("class", "box-whiskers-node").call(drawElement);
    },
  },
