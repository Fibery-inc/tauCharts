...
function exportTo(settings) {
  return {
    ...
    _toPng: function (chart) {
      this._createDataUrl(chart, (err, dataURL) => {
        if (err) {
    ...
    _renderSizeLegend: function (configUnit, svg, chart, width, offset) {
      ...
      if (last - first) {
        ...
        values = utils.unique(
          [first, first + step, first + step * 2, first + step * 3, last].map(function (x) {
      ...
      var maxDiameter = Math.max.apply(
        null,
        data.map(function (x) {
          return x.diameter;
        })
    ...
    init: function (chart) {
      ...
      this._container = chart.insertToHeader('<a class="tau-chart__export" style="' + menuStyle + '">Export</a>');
      ...
      const exportHandler = chart.on(
        "export-to",
        function (chart, { type, fileName }) {
          this._fileName = fileName || this._fileName;
          this._select(type, chart);
        }.bind(this)
      );
      const deprecatedExportHandler = chart.on(
        "exportTo",
        function (chart, type) {
          Taucharts.api.globalSettings.log(
