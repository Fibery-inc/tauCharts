...
function QuickFilter(xSettings = {}) {
  ...
  return {
    init: function (chart) {
      ...
      this._fields = Array.isArray(fields) && fields.length > 0 ? fields : Object.keys(sources.dims);
    ...
    _filterWrapper: utils.template(
      '<div class="tau-chart__filter__wrap">' + '<div class="tau-chart__legend__title"><%=name%></div>' + "</div>"
    ),
    ...
    _drawFilter: function (dim) {
      ...
      var x = d3.scaleLinear().domain(bounds).range([0, width]);
      ...
      var rect = svg
        ...
        .attr("transform", function (d) {
          return "translate(" + x(d) + "," + (margin.top + padding) + ")";
        })
        .attr("height", height)
        .attr("width", 1);
      ...
      brushg.append("g").attr("class", "resize e").attr("cursor", "ew-resize").attr("pointer-events", "none");
      brushg.append("g").attr("class", "resize w").attr("cursor", "ew-resize").attr("pointer-events", "none");
      ...
      brushg.selectAll("rect").attr("height", height + 2 * padding);
      ...
      function getFormatters(formatters) {
        var index = formatters.findIndex(function (token) {
    ...
    _applyFilter: function (dim) {
      ...
      if (REFRESH_DELAY < 0) {
        ...
        this._refreshRequestId = setTimeout(
          function () {
