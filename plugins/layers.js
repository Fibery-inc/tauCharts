...
function Layers(xSettings) {
  ...
  return {
    init: function (chart) {
      ...
      spec
        .setSettings("excludeNull", false)
        .setSettings("fitModel", null)
        .addScale(metaField, {
          type: "color",
          source: "/",
          dim: metaField,
          brewer: settings.brewer,
        })
        .addTransformation("slice-layer", function (data, props) {
          ...
          if (g) {
            return data.filter(function (row) {
              ...
              return groupKey && groupVal !== null && typeof groupVal !== "undefined";
            });
          } else {
            return data.filter(function (row) {
              return row[metaField] === layersText[k] && row[k] !== null && typeof row[k] !== "undefined";
            });
          }
        });

      var layersDims = [this.primaryY].concat(settings.layers).reduce(function (memo, layer) {
      ...
      chart.setupChartSourceModel(function (originalSources) {
        ...
        sources["/"].data = originalSources["/"].data.reduce(function (memo, row) {
          return memo.concat(
            layersDims.map(function (layerDim) {
              var seed = {};
    ...
    getLayersText: function () {
      return [this.primaryY].concat(settings.layers).reduce(
        function (memo, layer) {
      ...
      return function (memo, unit, parent) {
        ...
        if (self.isLeafElement(unit, parent)) {
          ...
          if (isPrimaryLayer && isNotEmptySizeScale) {
            ...
            fullSpec.addScale(sizeScaleName, {
              type: "size",
              source: "?",
              mid: 1,
            });
          ...
          pluginsSDK.unit(unit).addTransformation("slice-layer", params);
        }
    ...
    onSpecReady: function (chart, specRef) {
      ...
      if (!settings.showLayers || !self._isApplicable) {
        fullSpec.unit().traverse(function (unit, parentUnit) {
          if (self.isLeafElement(unit, parentUnit)) {
            pluginsSDK.unit(unit).addTransformation("defined-only", {
              key: fullSpec.getScale(unit.y).dim,
            });
          }
        });
        return;
      ...
      fullSpec = settings.layers.reduce(function (memo, layer) {
      ...
      var currUnit = self.buildLayersLayout(fullSpec).addFrame({
        key: { x: 1, y: 1 },
        units: [
          (cursor = pluginsSDK.unit(prevUnit.clone()))
    ...
    onUnitsStructureExpanded: function () {
      ...
      if (self._isApplicable && settings.mode === "merge") {
        ...
        var minMax = d3.extent(
          utils.flatten(
            Object.keys(hashBounds).map(function (key) {
