...
function labelBox(options) {
  ...
  var setValues = function (args) {
    ...
    var t = g.select(".tau-crosshair__label__text").attr("fill", color);
    var tAndBg = g
      .selectAll(".tau-crosshair__label__text, .tau-crosshair__label__text-shadow")
...
function Crosshair(xSettings) {
  ...
  var plugin = {
    ...
    _createNode: function () {
      var root = d3.select(document.createElementNS(svgNS, "g")).attr("class", "tau-crosshair");
    ...
    _setValues: function (xData, yData, colorData) {
      var setCrosshairGroupValues = function (data) {
        ...
        if ((data.dir === "x" && settings.xAxis) || (data.dir === "y" && settings.yAxis)) {
    ...
    _showCrosshair: function (e, unit, parentUnit) {
      ...
      if (unit.config.stack) {
        if (unit.config.flip) {
          ...
          ex = (xValue < 0 ? Math.min : Math.max).apply(
            null,
            xSameSign.map(function (d) {
          ...
          ey = (yValue < 0 ? Math.max : Math.min).apply(
            null,
            ySameSign.map(function (d) {
      ...
      var pad = (function getCrossPadding() {
        const paddingElements = ["ELEMENT.AREA", "ELEMENT.INTERVAL", "ELEMENT.INTERVAL.STACKED"];
