...
function ChartGeoMapLegend(xSettings) {
  ...
  var plugin = {
    ...
    _template: utils.template(
      '<div class="tau-chart__legend"><div class="tau-chart__legend__title"><%=name%></div><%=items%></div>'
    ),
    ...
    _drawColorLegend: function () {
      ...
      self._color.forEach(function (c) {
        var firstNode = self._chart.select(function (unit) {
          return unit.config.color === c;
        })[0];

        if (firstNode) {
          var colorScale = firstNode.getScale("color");
          var dataSource = self._chart.getDataSources({
            excludeFilter: ["legend"],
          });

          var domain = utils.unique(
            dataSource[colorScale.source].data.map(function (x) {
    ...
    _drawFillLegend: function () {
      ...
      self._fill.forEach(function (c) {
        var nodes = self._chart.select(function (unit) {
    ...
    _assignStaticBrewersOrEx: function () {
      var self = this;
      self._color.forEach(function (c) {
        var scaleConfig = self._chart.getSpec().scales[c];

        var fullLegendDataSource = self._chart.getDataSources({
          excludeFilter: ["legend"],
        });

        var fullLegendDomain = self._chart.getScaleFactory(fullLegendDataSource).createScaleInfoByName(c).domain();

        if (!scaleConfig.brewer) {
          scaleConfig.brewer =
            scaleConfig.dimType !== "measure"
