...
export default function DiffTemplate(tooltip: ElementTooltip, settings: TooltipSettings) {
  ...
  return Object.assign({}, base, {
    ...
    contentTemplate(args) {
      return [this.fieldsTemplate(args), this.tableTemplate(args)].join("\n");
    },
    ...
    filterFields(fields) {
      ...
      const filtered = fields.filter((field) => {
        return field !== scaleColor.dim && field !== scaleX.dim && field !== scaleY.dim;
      });
    ...
    fieldUpdownTemplate({ diff, sign }) {
      ...
      return [`<span class="${updownCls} ${updownSignCls}">`, `${updownSymbol}${diffVal}`, "</span>"].join("");
    },
