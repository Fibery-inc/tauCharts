...
import { d3Selection as Selection, GrammarElement } from "../../src/definitions";
...
const IntervalHighlight = <HighlightElement>{
  ...
  _drawRange(range: number[]) {
    ...
    function defineGradient() {
      ...
      defsEnter.append("stop").attr("class", START_CLS).attr("offset", "0%");

      defsEnter.append("stop").attr("class", END_CLS).attr("offset", "100%");
    }
    ...
    function drawGroups(): HighlightDataBinding {
      const g = container.selectAll(`.${ROOT_CLS}`).data(range ? [1] : []) as Selection;

      g.exit().remove();

      const gEnter = g.enter().append("g").attr("class", ROOT_CLS).attr("pointer-events", "none") as Selection;
    ...
    function drawRange({ g, gEnter }: HighlightDataBinding) {
      ...
      const rectEnter = gEnter.append("rect").attr("class", RANGE_CLS).attr("fill", `url(#${GRADIENT_ID})`);
      ...
      rectEnter.merge(rect).attr("x", x).attr("y", y).attr("width", Math.abs(width)).attr("height", Math.abs(height));
    }
    ...
    function drawStart({ g, gEnter }: HighlightDataBinding) {
      ...
      const lineEnter = gEnter.append("line").attr("class", START_CLS);
      ...
      lineEnter.merge(line).attr("x1", x1).attr("y1", y1).attr("x2", x2).attr("y2", y2);
    }
    ...
    function drawEnd({ g, gEnter }: HighlightDataBinding) {
      ...
      const lineEnter = gEnter.append("line").attr("class", END_CLS);
    ...
    utils.take(drawGroups()).then((binding) => {
      defineGradient();
      drawRange(binding);
      drawStart(binding);
      drawEnd(binding);
