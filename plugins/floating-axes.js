...
function floatingAxes(_settings) {
  ...
  return {
    init: function (chart) {
      ...
      this.beforeExportHandler = chart.on(
        "beforeExportSVGNode",
        function () {
      ...
      this.afterExportHandler = chart.on(
        "afterExportSVGNode",
        function () {
    ...
    createFloatingLayout: function () {
      ...
      var axes = (function () {
        var getAxesSelector = function (axis) {
          ...
          return [rootPart + axisPart, rootPart + ".cell.tau-active " + axisPart].join(", ");
        };
        return {
      ...
      var defs = (function createSVGDefinitions() {
        var defs = d3Svg.append("defs").attr("class", "floating-axes floating-axes__defs");
      ...
      function transferAxes(floating, axesInfo) {
        axesInfo.forEach(function (i) {
          ...
          if (animationSpeed) {
            g.attr("transform", translate(i.parentTransform.translate0.x, i.parentTransform.translate0.y))
              .transition()
      ...
      var corner = (function createCorner() {
        ...
        scrollManager.handleVisibilityFor(g, "xy").onScroll(function (scrollLeft, scrollTop) {
      ...
      var shadows = (function createShadows() {
        ...
        var g = d3Svg.append("g").attr("class", "floating-axes floating-axes__shadows").attr("pointer-events", "none");
        ...
        scrollManager
          ...
          .onScroll(function (scrollLeft, scrollTop) {
            ...
            toggle(shadowEW, scrollLeft + pos.visibleWidth < pos.svgWidth && pos.svgWidth > pos.visibleWidth);
            toggle(shadowSN, scrollTop + pos.visibleHeight < pos.svgHeight && pos.svgHeight > pos.visibleHeight);
      ...
      scrollManager.onScroll(function (scrollLeft) {
        svg.insertBefore(xAxesNode, scrollLeft === 0 ? cornerNode.nextElementSibling : yAxesNode);
      });
...
function getDynamicTransform(node) {
  var isTransformInTransition = node[storeProp] && node[storeProp].transform;
  var currentTransform = parseTransform(node.getAttribute("transform"));
  var nextTransform = isTransformInTransition ? parseTransform(node[storeProp].transform) : currentTransform;
  return {
      ...
      if (scrollPos !== prevScroll[d]) {
        ...
        scrollTimeout[d] = setTimeout(function () {
          callSelections(items[d], function (s) {
            ...
            s.transition("floatingAxes_scrollVisibility").duration(SHOW_SCROLL_ITEMS_DURATION).attr("opacity", 1);
          });
