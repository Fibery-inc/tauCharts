...
var stretchByOrdinalAxis = function (noteItem: AreaAnnotation | AxisAnnotation) {
  return function (model: GrammarModel) {
    ...
    res[method] = function (row) {
      ...
      if (scale.period) {
        const gen = Taucharts.api.tickPeriod.get(scale.period, {
          utc: scale.utcTime,
        });
...
function annotations(xSettings: AnnotationSettings) {
  ...
  return {
    init(chart: Plot) {
      ...
      specRef.transformations.dataRange = (data, metaInfo: MetaInfo) => {
        ...
        if (metaInfo.startText) {
          startBound.text = template(metaInfo.startText, {
            value: format(startBound[a]),
          });
        }
        if (metaInfo.endText) {
          endBound.text = template(metaInfo.endText, {
            value: format(endBound[a]),
          });
        }
      ...
      specRef.transformations.dataLimit = (data, metaInfo: MetaInfo) => {
        ...
        var from = primaryScaleInfo.period
          ? Taucharts.api.tickPeriod
              .get(primaryScaleInfo.period, { utc: specRef.settings.utcTime })
      ...
      specRef.transformations.lineNoteData = (data, metaInfo: LineMetaInfo) => {
        ...
        const xPeriod = xScale.period ? Taucharts.api.tickPeriod.get(xScale.period, { utc: xScale.utcTime }) : null;
        const yPeriod = yScale.period ? Taucharts.api.tickPeriod.get(yScale.period, { utc: yScale.utcTime }) : null;

        const points = metaInfo.points.map((d) => {
          return [xPeriod ? xPeriod.cast(d[0]) : d[0], yPeriod ? yPeriod.cast(d[1]) : d[1]];
        });
    ...
    addAreaNote: function (specRef: GPLSpec, coordsUnit: Unit, noteItem: AreaAnnotation) {
      ...
      var axes = noteItem.dim === xScale.dim ? ["x", "y"] : noteItem.dim === yScale.dim ? ["y", "x"] : null;
    ...
    addLineNote: function (specRef: GPLSpec, coordsUnit: Unit, noteItem: AxisAnnotation | LineAnnotation) {
      ...
      if (Array.isArray(noteItem.dim)) {
        isAxisNote = false;
        dims = noteItem.dim;
        if ((dims[0] === xScale.dim && dims[1] === yScale.dim) || (dims[0] === yScale.dim && dims[1] === xScale.dim)) {
          axes = ["x", "y"];
        }
      } else {
      ...
      var annotatedLine = {
        ...
        guide: {
          ...
          label: {
            ...
            position: isAxisNote
              ? ["r", "b", "keep-in-box"]
              : ["auto:avoid-label-edges-overlap", "auto:adjust-on-label-overflow", "keep-in-box"],
          },
          x: {
            fillGaps: false,
          },
      ...
      let extension = isAxisNote
        ...
        : (() => {
            const points = dims[0] === xScale.dim ? noteItem.val : noteItem.val.map((d) => d.slice().reverse());
            return {
              transformation: [
                {
                  type: "lineNoteData",
