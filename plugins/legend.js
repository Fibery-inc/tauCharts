...
const splitRealValuesEvenly = (values, count, funcType) => {
  ...
  const steps = utils.range(1, count - 1).map((i) => {
...
const removeRedundantZeros = (() => {
  ...
  return (str) => str.replace(zerosAfterDot, "$1").replace(zerosAfterNotZero, "$1$2");
})();
...
const getNumberFormatter = (start, end) => {
  ...
  return (num) => {
    ...
    const trailingDigits = Math.min((diffExp < 0 ? Math.abs(diffExp) : 0) + (numExp < diffExp ? 1 : 0), 20); // NOTE: `toFixed` accepts values between 0 and 20.
...
function ChartLegend(xSettings) {
  ...
  return {
    init(chart) {
      ...
      this._color = Object.keys(spec.scales)
        .reduce(reducer("color"), [])
        .filter((scale) => chart.getScaleInfo(scale).discrete);

      this._fill = Object.keys(spec.scales)
        .reduce(reducer("color"), [])
        .filter((scale) => !chart.getScaleInfo(scale).discrete);
      ...
      if (hasColorScales || hasFillScales || hasSizeScales) {
        ...
        if (hasColorScales) {
          _delegateEvent(this._container, "click", RESET_SELECTOR, (e, currentTarget) => {
            this._toggleLegendItem(currentTarget, "reset");
          });

          _delegateEvent(this._container, "click", COLOR_ITEM_SELECTOR, (e, currentTarget) => {
            const mode = e.ctrlKey || e.target.matches(COLOR_TOGGLE_SELECTOR) ? "leave-others" : "focus-single";
            this._toggleLegendItem(currentTarget, mode);
          });

          _delegateEvent(this._container, "mouseover", COLOR_ITEM_SELECTOR, (e, currentTarget) => {
            this._highlightToggle(currentTarget, true);
          });

          _delegateEvent(this._container, "mouseout", COLOR_ITEM_SELECTOR, (e, currentTarget) => {
    ...
    destroy() {
      ...
      Object.keys(filters).forEach((id) => chart.removeFilter(filters[id]));
    ...
    _getFormat(dim) {
      return this._formatters[dim] ? this._formatters[dim].format : (x) => String(x);
    },
    ...
    _drawFillLegend() {
      ...
      self._fill.forEach((c) => {
        const firstNode = self._chart.select((unit) => unit.config.color === c)[0];
        ...
        if (firstNode) {
          ...
          const numDomain = isDate ? domain.map(Number) : domain;
          ...
          const formatter = isDate ? dateFormatter : numFormatter;
          ...
          const labelsCount = !fillScale.isInteger
            ? 3
            : (numDomain[1] - numDomain[0]) % 3 === 0
            ? 4
            : (numDomain[1] - numDomain[0]) % 2 === 0
            ? 3
            : 2;
          ...
          self._container.insertAdjacentHTML(
            "beforeend",
            self._template({
          ...
          const container = self._container.lastElementChild.querySelector(".tau-chart__legend__gradient-wrapper");
          const width = container.getBoundingClientRect().width;
          const totalLabelsW = labels.reduce((sum, label) => sum + getTextWidth(label), 0);
          let isVerticalLayout = false;
          if (totalLabelsW > width) {
            if (labels.length > 1 && getTextWidth(labels[0]) + getTextWidth(labels[labels.length - 1]) > width) {
          ...
          const stops = splitEvenly(numDomain, brewerLength).map((x, i) => {
          ...
          container.insertAdjacentHTML("beforeend", gradient);
        }
      });
    },
    ...
    _drawSizeLegend() {
      ...
      self._size.forEach((c) => {
        const firstNode = self._chart.select((unit) => unit.config.size === c)[0];
        ...
        if (firstNode) {
          ...
          const container = self._container.lastElementChild.querySelector(".tau-chart__legend__size-wrapper");
          const width = container.getBoundingClientRect().width;
          ...
          const layout = isVerticalLayout
            ...
            : (() => {
                const padL = Math.max(getTextWidth(labels[0]) / 2, sizes[0] / 2);
                const padR = Math.max(getTextWidth(labels[labels.length - 1]) / 2, sizes[sizes.length - 1] / 2);
                const gap =
                  (width -
                    sizes.reduce((sum, n, i) => {
          ...
          container.insertAdjacentHTML("beforeend", sizeLegend);
        }
      });
    },
    _getLegendColorByScales() {
      ...
      return self._color.reduce((legendColorsInfo, c) => {
        ...
        if (firstNode) {
          ...
          const dataSource = self._chart.getDataSources({
            excludeFilter: ["legend"],
          });

          let domain = utils.unique(dataSource[colorScale.source].data.map((x) => x[colorScale.dim]));
    ...
    _toggleLegendItem(target, mode) {
      const filters = this._currentFilters;
      const colorNodes = target
        ? Array.prototype.filter.call(target.parentNode.childNodes, (el) => el.matches(COLOR_ITEM_SELECTOR))
        : null;
      ...
      const setGuideBackground = (node, visible) => {
        node.querySelector(".tau-chart__legend__guide").style.backgroundColor = visible ? "" : "transparent";
      };
    ...
    _highlightToggle(target, doHighlight) {
      ...
      const isRowMatch = doHighlight ? createIsRowMatchInterceptor(dim, val) : (row) => null;

      this._chart
        .select(
          (
            unit // return unit.config.color === scaleId;
          ) =>
    ...
    _assignStaticBrewersOrEx() {
      ...
      self._color.forEach((c) => {
        const scaleConfig = self._chart.getSpec().scales[c];

        const fullLegendDataSource = self._chart.getDataSources({
          excludeFilter: ["legend"],
        });

        const fullLegendDomain = self._chart.getScaleFactory(fullLegendDataSource).createScaleInfoByName(c).domain();
