...
describe("Bar as Span plugin", function () {
  ...
  it("should place spans correct", function () {
    ...
    var facets = [
      [["Broken tabs layout", "Modal dialog collapsed"], ["Payment form"], ["Scroll bar"]],
      [["Tractors", "Nails"], ["Oil"]],
    ];
    ...
    facets.forEach(function (grid) {
      grid.forEach(function (row, i) {
        var firstCell = row[0];
        assert.equal(
          row.every((cell) => spanMap[cell].y === spanMap[firstCell].y),
          true,
          "all spans at row have equal Y"
        );
        assert.equal(
          row.every((cell) => labelMap[cell].y === labelMap[firstCell].y),
          true,
          "all labels at row have equal Y"
        );
        assert.equal(
          row.slice(1).every((cell, i) => {
            return spanMap[cell].x > spanMap[firstCell].x;
          }),
          true,
          "next span at row has larger X"
        );
        assert.equal(
          row.slice(1).every((cell, i) => {
          ...
          true,
          "next label at row has larger X"
        );
        if (i > 0) {
          var prevRowCell = grid[i - 1][0];
          assert.equal(
            spanMap[firstCell].y > spanMap[prevRowCell].y,
            true,
            "next row span Y is greater than prev row Y"
          );
          assert.equal(
            labelMap[firstCell].y > labelMap[prevRowCell].y,
            true,
            "next row label Y is greater than prev row Y"
          );
        }
      });
    });
  ...
  it("should place spans correct (no collapse)", function () {
    ...
    Object.keys(groups).forEach((key) => {
      ...
      assert.equal(
        group.slice(1).every((node, i) => {
          var s = d3.select(node);
          var p = d3.select(group[i]);
          return Number(s.attr("x")) > Number(p.attr("x")) && Number(s.attr("y")) > Number(p.attr("y"));
        }),
        true,
        "every next date is aligned bottom-right from previous"
      );
    });
  });
});
