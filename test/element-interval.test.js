...
unitsRegistry.reg("COORDS.RECT", Cartesian).reg("ELEMENT.INTERVAL", Interval);
  ...
  bars.forEach(function (bar, index) {
    Array.from(bar.childNodes).forEach(function (el, ind) {
      ...
      if (coords[index][ind].hasOwnProperty("width")) {
        expect(convertToFixed(attrib(el, "width"))).to.equal(
          convertToFixed(coords[index][ind].width),
          `width (${index} / ${ind})`
        );
      }

      if (coords[index][ind].hasOwnProperty("height")) {
        expect(convertToFixed(attrib(el, "height"))).to.equal(
          convertToFixed(coords[index][ind].height),
          `height (${index} / ${ind})`
        );
      }
    });
...
describePlot(
  "ELEMENT.INTERVAL.FLIP WITH LINEAR AND CATEGORICAL AXIS",
  {
    unit: {
      type: "COORDS.RECT",
      x: "y",
...
var testExpectCoordForTimeAdCount = [[800, 400, 1]];
    ...
    it("should group contain interval element", function () {
      ...
      bars.forEach(function (bar, barIndex) {
        Array.from(bar.childNodes).forEach(function (el, elIndex) {
          expect(roundAttr(el, "height")).to.equal(testExpectCoordForTimeAdCount[barIndex][elIndex]);
        });
      });
    });
  }
...
describePlot(
  ...
  function () {
    it("should group contain interval element", function () {
      ...
      bars.forEach(function (bar, barIndex) {
        Array.from(bar.childNodes).forEach(function (el, elIndex) {
          expect(roundAttr(el, "width")).to.equal(testExpectCoordForTimeAdCount[barIndex][elIndex]);
        });
      });
    });
  }
...
describePlot(
  ...
  function () {
    it("should group contain interval element", function () {
      ...
      var coords = [[400, 200, minimalHeight, 0, minimalHeight, 200, 400]];
...
describeChart(
  "Horizontal bars sorted by width",
  {
    type: "horizontal-bar",
    x: "x",
    y: "y",
