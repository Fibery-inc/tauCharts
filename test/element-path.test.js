...
describe("ELEMENT.PATH", function () {
  ...
  it("should render two area elements", function () {
    ...
    // 1 path
    expect(d3.select(svgPolygons[0]).attr("points")).to.equal(`${x0},${y0} ${x1},${y1}`);

    // 2 path
    expect(d3.select(svgPolygons[1]).attr("points")).to.equal(`${x0},${y1} ${x1},${y0}`);
...
describe("ELEMENT.PATH with text assigned", function () {
  ...
  it("should render polygon with text on each angle", function () {
    ...
    expect(str(d3.rgb(polygon.attr("fill")))).to.be.equal(str(d3.rgb("rgb(171, 205, 239)")));
