...
describe("scales-factory", function () {
  it("should support flex scales", function () {
    ...
    var f = new ScalesFactory(ScalesRegistry.instance(), {
      "/": {
        dims: {
          x1: {
            type: "ordinal",
      ...
      .createScaleInfo({
        ...
        ratio: function (key, size, varSet) {
          ...
          var xHash = (keys) => {
            return utils
              .unique(
                data.map((row) => keys.reduce((r, k) => r.concat(row[k]), [])),
                (t) => JSON.stringify(t)
              )
              .reduce((memo, t) => {
                var k = t[0];
...
describe("scales", function () {
  ...
  it("should support [time] scale", function () {
    ...
    var scale3 = new TimeScale(xSrcUTC, {
      dim: "t",
      utcTime: true,
    }).create([0, 100]);
  ...
  it("time scale with single value domain should add 1 hour range around it when nice is applied", function () {
    ...
    var interval = [myDate.getTime() - oneDay, myDate.getTime() + oneDay];
    var expectedRange = d3.scaleTime().domain(interval).nice().domain();

    expect(scaleSingleValue.domain().map((x) => x.getTime())).to.deep.equal(expectedRange.map((x) => x.getTime()));
  });
  ...
  it("should set intervals for time scale", function () {
    ...
    expect(dayScale.domain().map((x) => x.getTime())).to.deep.equal(
      [new Date("2017-01-01T00:00Z"), new Date("2017-01-04T00:00Z")].map((d) => d.getTime()),
      "domain rounded to day"
    );

    expect(monthScale.domain().map((x) => x.getTime())).to.deep.equal(
      [new Date("2016-12-01T00:00Z"), new Date("2017-02-01T00:00Z")].map((d) => d.getTime()),
      "domain with single month extended with 1 interval"
    );
  });
  ...
  it("should extend time scale with intervals when single value", function () {
    ...
    expect(dayScale.ticks(10)).to.deep.equal([
      new Date("2017-01-01T00:00Z"),
      new Date("2017-01-02T00:00Z"),
      new Date("2017-01-03T00:00Z"),
      new Date("2017-01-04T00:00Z"),
    ]);
  });

  it("should generate periodic time ticks with fallback for large ticks count", function () {
    var data = [{ t: new Date("2017-01-01T10:00Z") }, { t: new Date("2017-04-01T02:00Z") }];
    var dayScale = new TimeScale(
      {
        part: () => data.slice(),
        full: () => data.slice(),
    ...
    expect(dayScale.ticks(5)).to.deep.equal([
      new Date("2017-01-01T00:00Z"),
  ...
  it("should support [period] scale", function () {
    var scale0 = new PeriodScale(xSrc, {
      dim: "t",
      period: "day",
    }).create([0, 100]);
    ...
    var scale1 = new PeriodScale(xSrc, {
      dim: "t",
      min: "2015-04-15",
      max: "2015-04-20",
      period: "day",
    ...
    var scale2 = new PeriodScale(xSrc, {
      dim: "t",
      period: "day",
      fitToFrameByDims: [],
    }).create([0, 100]);
    ...
    var scale3 = new PeriodScale(xSrc, {
      dim: "t",
      ratio: scale3Ratio,
      fitToFrameByDims: [],
    }).create([0, 100]);
    ...
    var scale4 = new PeriodScale(xSrc, {
      dim: "t",
      ratio: scale4Ratio,
      fitToFrameByDims: [],
    }).create([0, 100]);
    ...
    var scale5 = new PeriodScale(xSrcUTC, {
      dim: "t",
      period: "day",
      utcTime: true,
    }).create([0, 100]);
  ...
  it("should support [linear] scale", function () {
    var scale0 = new LinearScale(xSrc, {
      dim: "i",
    }).create([0, 100]);
    ...
    var scale1 = new LinearScale(xSrc, {
      dim: "i",
      min: -10,
      max: 10,
    }).create([0, 100]);
    ...
    var scale1A = new LinearScale(xSrc, {
      dim: "i",
      min: -19,
      max: 19,
      nice: true,
    ...
    var scale2 = new LinearScale(xSrc, {
      dim: "i",
      nice: true,
    }).create([0, 100]);
  ...
  it("should support [logarithmic] scale", function () {
    ...
    var xLog10NegativeSrc = new DataSource(
      xLog10Src.data.map((d) => {
        return { i: -d.i, s: d.s, x: d.x, t: d.t };
      })
    );

    var scale0 = new LogarithmicScale(xLog10Src, {
      dim: "i",
    }).create([0, width]);
    ...
    expect(scale0.ticks(5)).to.deep.equal([0.5, 20, 300, 2000, 10000, 40000, 100000]);

    expect(scale0.hasOwnProperty("stepSize")).to.equal(true);
    expect(scale0.stepSize()).to.equal(0);

    var scale1 = new LogarithmicScale(xLog10Src, {
      dim: "i",
      nice: true,
    }).create([0, width]);
    ...
    var scale2 = new LogarithmicScale(xLog10NegativeSrc, {
      dim: "i",
      nice: true,
    }).create([0, width]);
    ...
    expect(function () {
      var scale3 = new LogarithmicScale(xLog10Src, {
        dim: "i",
        min: crossLow,
        max: crossHigh,
        nice: true,
  ...
  it("should support [color] scale", function () {
    ...
    var scale0 = new ColorScale(xSrc, {
      dim: "x",
      order: ["low", "medium", "high"],
    }).create();
    ...
    var scale1 = new ColorScale(xSrc, {
      dim: "x",
      order: ["low", "medium", "high"],
      brewer: ["c1", "c2", "c3"],
    }).create();
    ...
    var scale2 = new ColorScale(xSrc, {
      dim: "x",
      order: ["low", "medium", "high"],
    ...
    var scale3 = new ColorScale(xSrc, {
      dim: "x",
      order: ["low", "medium", "high"],
      brewer: function (v) {
        return "len-" + v.length;
  ...
  it("should support [ordinal] scale", function () {
    var scale0 = new OrdinalScale(xSrc, {
      dim: "x",
      order: ["low", "medium", "high"],
    }).create([0, 90]);
    ...
    var scale1 = new OrdinalScale(xSrc, {
      dim: "x",
      order: ["low", "medium", "high"],
      ratio: {
        low: 0.5,
    ...
    var scale2 = new OrdinalScale(xSrc, {
      dim: "x",
      order: ["low", "medium", "high"],
      ratio: function (x) {
        var map = {
