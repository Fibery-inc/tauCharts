...
describe("Bar chart", function () {
  ...
  it("should draw labels after bars", function () {
    ...
    var firstLabelIndex = children.findIndex(function (b) {
  ...
  it("should put labels inside and outside of horizontal bars", function () {
    ...
    var bars = toArray(svg.querySelectorAll(".bar")).map((el) => el.getBoundingClientRect());
    var labels = toArray(svg.querySelectorAll(".i-role-label")).map((el) => el.getBoundingClientRect());

    assert.equal(bars.length, data.length);
    assert.equal(labels.length, data.length);

    assert.equal(
      labels.every((l) => l.left >= grid.left && l.right <= grid.right),
      true,
      "labels do not exceed grid"
    );
    ...
    assert.equal(
      labels.every((l) => {
        ...
        return !((l.left < b.left && l.right > b.left) || (l.left < b.right && l.right > b.right));
      }),
      true,
      "labels do not intersect bars"
    );
  ...
  it("should put labels inside and outside of vertical bars", function () {
    ...
    assert.equal(
      labels.every((l) => l.top >= grid.top && l.bottom <= grid.bottom),
      true,
      "labels do not exceed grid"
    );
    ...
    assert.equal(
      labels.every((l) => {
        ...
        return !((l.top < b.top && l.bottom > b.top) || (l.top < b.bottom && l.bottom > b.bottom));
      }),
      true,
      "labels do not intersect bars"
    );
