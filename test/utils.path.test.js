...
describe("path utilities", function () {
  ...
  it("should return SVG path value", function () {
    ...
    expect(lines.getPolyline(points)).to.equal(["M0,30", "L30,0", "L60,30"].join(" "));
    expect(lines.getCurve(toCurve(points))).to.equal(
      ["M0,30", "C10,10", "20,0", "30,0", "C40,0", "50,10", "60,30"].join(" ")
    );
  ...
  it("should return SVG area path value", function () {
    ...
    expect(getAreaPolygon(polyDir, polyRev)).to.equal(["0,20", "30,0", "60,20", "60,40", "30,20", "0,60"].join(" "));
    expect(getSmoothAreaPath([], [])).to.equal("");
    expect(getSmoothAreaPath(curveDir, curveRev)).to.equal(
      ["M0,0", "C30,0", "60,20", "90,20", "L90,80", "C60,80", "30,20", "0,20", "Z"].join(" ")
    );
  });
  ...
  it("should interpolate path points", function () {
    ...
    expect(interpolator7(0.5)).to.deep.equal([a[0], { id: 2, x: 10, y: 10, positionIsBeingChanged: true }, a[2]]);

    var interpolator8 = createInterpolator([a[0], { id: 7, x: 10, y: 40 }, a[2]], a.slice(0, 3));
    expect(interpolator8(0.5)).to.deep.equal([a[0], { id: a[1].id, x: 10, y: 30, positionIsBeingChanged: true }, a[2]]);
  });
  ...
  it("should render interpolated area", function () {
    ...
    const coords = Array.from(pathValue.match(/\d+\.?\d+,\d+\.?\d+/g)).map((c) => {
...
describe("brush-line-builder", function () {
  it("should return line with variable width", function () {
    ...
    expect(singlePoint1).to.be.equal(["M100,80", "A20,20 0 0 1 100,120", "A20,20 0 0 1 100,80", "Z"].join(" "));

    var singlePoint2 = getBrushLine([
      { x: 90, y: 100, size: 20 },
      { x: 100, y: 100, size: 40 },
    ]);
    expect(singlePoint2).to.be.equal(["M100,80", "A20,20 0 0 1 100,120", "A20,20 0 0 1 100,80", "Z"].join(" "));
  ...
  it("should return curve with variable width", function () {
    ...
    var singlePoint2 = getBrushCurve(
      toCurve([
        { x: 90, y: 100, size: 20 },
        { x: 100, y: 100, size: 40 },
      ])
