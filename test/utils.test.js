...
describe("utils helper", function () {
  ...
  it("should nice time domain", function () {
    var samples = [
      ...
      [
        ["2004-09-03", "2012-03-01"],
        ["2004-09-03", "2012-03-01"],
      ],
      [
        ["2004-03-01", "2012-09-01"],
        ["2004-01-01", "2013-01-01"],
      ],
      [
        ["2012-09-03", "2012-09-03"],
        ["2012-09-02", "2012-09-04"],
      ],
    ];
    checkTime(samples);
  });
  ...
  it("should allow to create ratio function for level 2 facet (no inner padding)", function () {
    var fx = utils.generateRatioFunction("x", ["x1", "x2"], createLiveSpec());

    expect(fx("a1", 100, ["a1", "a2"])).to.equal(0.8);
    expect(fx("a2", 100, ["a1", "a2"])).to.equal(0.2);

    var fy = utils.generateRatioFunction("y", ["x1", "x2"], createLiveSpec({ l: 0, r: 0, t: 0, b: 0 }));
  ...
  it("should allow to create ratio function for level 2 facet (inner padding)", function () {
    var fx = utils.generateRatioFunction("x", ["x1", "x2"], createLiveSpec({ l: 10, r: 10, t: 0, b: 0 }));

    expect(fx("a1", 100, ["a1", "a2"])).to.equal(0.68);
    expect(fx("a2", 100, ["a1", "a2"])).to.equal(0.32);

    var fy = utils.generateRatioFunction("y", ["x1", "x2"], createLiveSpec({ l: 0, r: 0, t: 10, b: 10 }));
...
describe("utils-draw", function () {
  it("should support isIntersect method", function () {
    var x0 = drawUtils.isIntersect(0, 0, 1, 1, 1, 0, 2, 1);

    expect(x0).to.equal(false);

    var x1 = drawUtils.isIntersect(0, 0, 1, 1, 1, 0, 0, 1);
...
describe("d3 decorators", function () {
  ...
  beforeEach(function () {
    ...
    svgNode = d3.select(div).append("svg").attr("width", 100).attr("height", 100);
  });
  ...
  it("should support `avoidTickTextCollision` method", function () {
    ...
    ticks.each(function () {
      ...
      lineAfter.push([lineRef.attr("y1"), lineRef.attr("y2")]);
    });
    expect(ticks.size()).to.equal(domain.length, "Ticks created");
