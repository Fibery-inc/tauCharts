...
describe("ELEMENT.AREA", function () {
  ...
  it("should render two area elements", function () {
    ...
    // up triangle
    expect(d3.select(svgPolygons[0]).attr("points")).to.equal("0,1000 1000,0 1000,1000 0,1000", "up triangle");

    // down triangle
    expect(d3.select(svgPolygons[1]).attr("points")).to.equal("0,0 1000,1000 1000,1000 0,1000", "down triangle");
...
describe("ELEMENT.AREA min is negative", function () {
  ...
  it("should render two area elements", function () {
    ...
    // down triangle
    expect(d3.select(svgPolygons[0]).attr("points")).to.equal("0,0 1000,500 1000,500 0,500", "down triangle");

    // "bantik" :)
    expect(d3.select(svgPolygons[1]).attr("points")).to.equal("0,0 1000,1000 1000,500 0,500", "bantik");
  });
});
...
describe("ELEMENT.AREA max is negative", function () {
  ...
  it("should render two area elements", function () {
    ...
    expect(d3.select(svgPolygons[0]).attr("points")).to.equal(
      "0,0 1000,1000 1000,0 0,0",
      "triangle to the negative area"
    );
  });
});
...
describe("ELEMENT.AREA flipped", function () {
  ...
  it("should render two area elements", function () {
    ...
    // up triangle (flipped)
    expect(d3.select(svgPolygons[0]).attr("points")).to.equal(
      "0,1000 1000,0 1000,1000 0,1000",
      "up triangle (flipped)"
    );

    // down triangle (flipped)
    expect(d3.select(svgPolygons[1]).attr("points")).to.equal(
      "0,0 1000,1000 1000,1000 0,1000",
      "down triangle (flipped)"
    );
...
describe("ELEMENT.AREA within ordinal / measure scales", function () {
  ...
  it("should render two area elements", function () {
    ...
    // up triangle
    expect(d3.select(svgPolygons[0]).attr("points")).to.equal("0,250 1000,750 1000,500 0,500", "up triangle");

    // down triangle
    expect(d3.select(svgPolygons[1]).attr("points")).to.equal("1000,250 0,750 0,500 1000,500", "down triangle");
...
describe("area chart with split parameter", function () {
  ...
  it("should render area elements by split and color", function () {
    ...
    expect(d3.select(svgPolygons[2]).attr("points")).to.equal("0,1000 1000,1000 1000,1000 0,1000", "lower line");

    expect(d3.select(svgPolygons[1]).attr("points")).to.equal("0,500 1000,500 1000,1000 0,1000", "middle line");

    expect(d3.select(svgPolygons[0]).attr("points")).to.equal("0,0 1000,0 1000,1000 0,1000", "high line");
