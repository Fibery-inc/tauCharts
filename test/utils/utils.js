...
function destroyCharts() {
  Taucharts.Chart.winAware.slice(0).forEach(function (chart) {
    chart.destroy();
  });
}

function roundNumbersInString(str, fractionDigits) {
  return str.replace(/[-+]?(?:\d+\.?\d*|\.?\d+)(?:[e][-+]?\d+)?/gim, function (match) {
...
export default {
  ...
  simulateEvent: function (name, element, clientX = 0, clientY = 0) {
    ...
    evt.initMouseEvent(name, true, true, window, 0, 0, 0, clientX, clientY, false, false, false, false, 0, null);
