...
describe("Plugins SDK", function () {
  ...
  it("should support [spec.setSettings] method", function () {
    ...
    sdk.spec(specRef).setSettings("testSettings", "silent").setSettings("test2", "test-value");
  ...
  it("should support [spec.getSourceData] method", function () {
    ...
    expect(sdk.spec(specRef).getSourceData("test")).to.equal(specRef.sources.test.data);

    expect(sdk.spec(specRef).getSourceData("unknown")).to.deep.equal([]);
  });
  ...
  it("should support [spec.getSourceDim] method", function () {
    ...
    expect(sdk.spec(specRef).getSourceDim("test", "a")).to.equal(specRef.sources.test.dims.a);

    expect(sdk.spec(specRef).getSourceDim("test", "b")).to.deep.equal({});

    expect(sdk.spec(specRef).getSourceDim("unknown", "a")).to.deep.equal({});
  });
  ...
  it("should support [unit.addTransformation] method", function () {
    ...
    var unitRef2 = {
      type: "COORDS.RECT",
      transformation: [{ type: "noop", args: null }],
    };
