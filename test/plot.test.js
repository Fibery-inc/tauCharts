...
describe("tauChart.Plot", function () {
  ...
  it("should render to target with size (where target = element)", function () {
    new tauChart.Plot(spec).renderTo(document.getElementById("test-div"), { width: 1000, height: 1000 });
  ...
  it("should render to target with size (where target = ID selector)", function () {
    new tauChart.Plot(spec).renderTo("#test-div", { width: 2000, height: 1000 });
  ...
  it("should warn about rendering timeout", function (done) {
    ...
    chart.on("renderingtimeout", function () {
      ...
      switch (timeoutCount) {
        ...
        case 2:
          // Click "Cancel"
          utils.simulateEvent("click", svg.querySelector("." + CSS_PREFIX + "rendering-timeout-cancel-btn"));
          expect(TaskRunner.runnersInProgress).to.equal(0);
          chart.refresh();
          break;
        case 3:
          // Click "Continue"
          utils.simulateEvent("click", svg.querySelector("." + CSS_PREFIX + "rendering-timeout-continue-btn"));
          break;
      }
    });
