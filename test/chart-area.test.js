...
describe("Area plot chart", function () {
  ...
  it("should support [stack] transformation", function () {
    ...
    expect(round(d3.select(svgPolygons[2]).attr("points"))).to.equal(
      "0,600 500,400 1000,200 1000,600 500,600 0,600",
      "A"
    );

    expect(testUtils.hasClass(svgPolygons[0].parentNode, "B")).to.equal(true);
    expect(round(d3.select(svgPolygons[0]).attr("points"))).to.equal(
      "0,200 500,0 1000,200 1000,200 500,400 0,600",
      "B positive"
    );

    expect(testUtils.hasClass(svgPolygons[1].parentNode, "B")).to.equal(true);
    expect(round(d3.select(svgPolygons[1]).attr("points"))).to.equal(
      "0,1000 500,1000 1000,600 1000,600 500,600 0,600",
      "B negative"
    );
