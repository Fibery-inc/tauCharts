...
describe("Line plot chart", function () {
  ...
  it("should sort data by X if there is a func relation", function () {
    ...
    var spec = line.getSpec();
    expect(spec.sources["/"].data.map((r) => `${r.color}${r.i}`)).to.deep.equal(["A2", "B1", "A1", "B2", "A0", "B0"]);
  });
  ...
  it("should sort data by X if [horizontal] orientation is specified", function () {
    ...
    var spec = line.getSpec();
    expect(spec.sources["/"].data.map((r) => `${r.color}${r.i}`)).to.deep.equal(["A0", "A2", "B0", "B1", "A1", "B2"]);
  });
  ...
  it("should NOT sort data if [none] orientation is specified", function () {
    ...
    var spec = line.getSpec();
    expect(spec.sources["/"].data.map((r) => `${r.color}${r.i}`)).to.deep.equal(["A0", "A1", "A2", "B0", "B1", "B2"]);
  });
  ...
  it("should NOT sort data if [category] variable is used for X", function () {
    ...
    var spec = line.getSpec();
    expect(spec.sources["/"].data.map((r) => r.i)).to.deep.equal([0, 1, 2]);
  });
  ...
  it("should sort data if [ordered] variable is used for X", function () {
    ...
    var spec = line.getSpec();
    expect(spec.sources["/"].data.map((r) => r.i)).to.deep.equal([1, 2, 0, 3]);
  });
  ...
  it("should add data points for missing periods (fillGaps)", function () {
    ...
    const paths = Array.from(element.querySelectorAll(".tau-chart__line path")).reduce((map, el) => {
  ...
  it("should use empty size for missing periods (fillGaps)", function () {
    ...
    const paths = Array.from(element.querySelectorAll(".i-role-path path")).reduce((map, el) => {
      const s = d3.select(el);
      const path = round(s.attr("d"));
      const g = s.data()[0][0].group;
      map[g] = path;
    ...
    expect(paths.a).to.equal(
      "M0,508 L181,16 A24,24 0 1 1 226,34 L9,512 A5,5 0 0 1 0,508 Z M204,0 L403,0 A24,24 0 0 1 403,49 L204,49 A24,24 0 0 1 204,0 Z"
    );
    expect(paths.b).to.equal(
      "M9,508 L208,993 A5,5 0 0 1 199,997 L0,512 A5,5 0 0 1 9,508 Z M199,993 L398,508 A5,5 0 0 1 407,512 L208,997 A5,5 0 0 1 199,993 Z M403,505 L602,505 A5,5 0 0 1 602,515 L403,515 A5,5 0 0 1 403,505 Z M597,508 L778,16 A24,24 0 1 1 823,34 L606,512 A5,5 0 0 1 597,508 Z"
    );
