...
import { ChartSettings, GPLSpec, ScaleGuide, SpecTransformer, Unit, UnitGuide } from "./definitions";
...
var applyCustomProps = (targetUnit: Unit, customUnit: Unit) => {
  ...
  Object.assign(
    targetUnit.guide,
    Object.keys(guide).reduce((obj, k) => {
...
var extendLabel = function (guide: UnitGuide, dimension: string, extend?) {
  ...
  guide[dimension].label = utils.defaults(guide[dimension].label, extend || {}, {
    padding: 32,
    rotate: 0,
    textAnchor: "middle",
    cssClass: "label",
...
var extendAxis = function (guide: UnitGuide, dimension: "x" | "y", extend?) {
  guide[dimension] = utils.defaults(guide[dimension], extend || {}, {
    padding: 0,
    density: 30,
    rotate: 0,
    tickPeriod: null,
...
var getTextAnchorByAngle = (xAngle: number, xOrY = "x") => {
  ...
  var xRules: [number, number, string][] =
    xOrY === "x"
      ? [
          [0, 45, "middle"],
          [45, 135, "start"],
...
function calcXYGuide(
  ...
  guide.paddingNoTicks = Object.assign({}, guide.paddingNoTicks, {
    b: guide.x.hide
      ? paddingNoTicksB
      : sum([guide.x.padding, kxLabelW * (settings.distToXAxisLabel + settings.xFontLabelHeight + bottomBorder)]),
    l: guide.y.hide
      ? paddingNoTicksL
      : sum([guide.y.padding, kyLabelW * (settings.distToYAxisLabel + settings.yFontLabelHeight)]),
  });
...
var SpecEngineTypeMap: SpecEngines = {
  NONE: (srcSpec, meta, settings) => {
    ...
    fnTraverseSpec(utils.clone(spec.unit), spec.unit, (selectorPredicates, unit) => {
  ...
  "BUILD-LABELS": (srcSpec, meta) => {
    ...
    utils.traverseJSON(spec.unit, "units", createSelectorPredicates, (selectors, unit: Unit) => {
      if (selectors.isLeaf) {
        return unit;
      }
  ...
  "BUILD-GUIDE": (srcSpec, meta, settings) => {
    var spec = utils.clone(srcSpec);
    fnTraverseSpec(utils.clone(spec.unit), spec.unit, (selectorPredicates, unit: Unit) => {
      ...
      unit.guide = Object.assign(unit.guide, {
        showGridLines: unit.guide.hasOwnProperty("showGridLines")
          ...
          : selectorPredicates.isLeafParent
          ? "xy"
          : "",
      });
...
export class SpecTransformAutoLayout implements SpecTransformer {
  ...
  transform(chart: Plot) {
    ...
    var rule = spec.settings.specEngine.find((rule) => size.width <= rule.width || size.height <= rule.height);

    return SpecEngineFactory.get(rule.name, spec.settings, spec, (type, alias) =>
      chart.getScaleInfo(alias || `${type}:default`)
    );
  }
}
