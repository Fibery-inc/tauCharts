...
export class LinearScale extends BaseScale {
  ...
  constructor(xSource: DataFrame, scaleConfig: ScaleConfig) {
    ...
    vars = [Math.min(...[min, vars[0]].filter(Number.isFinite)), Math.max(...[max, vars[1]].filter(Number.isFinite))];
  ...
  extendScale(scale: d3.ScaleLinear<number, number>) {
    ...
    Object.assign(scale, {
      ...
      ticks: this.getField("isInteger") ? (n) => d3ScaleTicks.call(scale, n).filter(Number.isInteger) : scale.ticks,
    });
