...
export class TimeScale extends BaseScale {
  ...
  constructor(xSource: DataFrame, scaleConfig: ScaleConfig) {
    ...
    var vars = this.vars;
    const period = props.period ? UnitDomainPeriodGenerator.get(this.scaleConfig.period, { utc: props.utcTime }) : null;

    const domain = (d3.extent(vars) as [Date, Date]).map(period ? (v) => period.cast(new Date(v)) : (v) => new Date(v));

    const min = props.min == null ? domain[0] : new Date(props.min).getTime();
    const max = props.max == null ? domain[1] : new Date(props.max).getTime();

    vars = [new Date(Math.min(min as number, Number(domain[0]))), new Date(Math.max(max as number, Number(domain[1])))];
    ...
    if (props.nice && !period) {
      ...
      var d3TimeInterval =
        niceInterval && getD3Interval(niceInterval)
          ? props.utcTime
            ? getD3UtcInterval(niceInterval)
            : getD3Interval(niceInterval)
          : null;
      if (d3TimeInterval) {
  ...
  create(interval) {
    ...
    const d3Scale = d3TimeScale().domain(varSet).range(interval);
...
function getPeriodTicks(domain: Date[], period: string, utc: boolean, count = 10) {
  ...
  if (periodCount <= count) {
    return UnitDomainPeriodGenerator.generate(start, end, period, { utc }).filter((t) => t >= start && t <= end);
  }
...
function getTimeTicks(domain: Date[], utc: boolean, count = 10) {
  ...
  return interval.every(step).range(new Date(d0), new Date(d1 + 1));
}
