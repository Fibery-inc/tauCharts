...
import { GPLSpec, Unit as SpecUnit, ScaleGuide, AxisLabelGuide } from "./definitions";
...
class PluginsSDK {
  ...
  static traverseSpec(spec: GPLSpec, iterator: (node: SpecUnit, parent: SpecUnit) => void) {
    const traverse = (node: SpecUnit, fnIterator: (node: SpecUnit, parent: SpecUnit) => void, parentNode: SpecUnit) => {
  ...
  static extractFieldsFormatInfo(spec: GPLSpec) {
    ...
    return Object.keys(summary).reduce(function (memo, k) {
      ...
      const format = chosenFormat === "x-time-auto" ? (spec.settings.utcTime ? "day-utc" : "day") : chosenFormat;
      var nonVal = memo[k].nullAlias;
      var fnForm = format ? FormatterRegistry.get(format, nonVal) : (raw) => (raw == null ? nonVal : String(raw));
  ...
  static getFieldFormatters(spec: GPLSpec, formatters: { [field: string]: Formatter }) {
    ...
    Object.keys(formatters).forEach((k) => {
      ...
      if (fmt.hasOwnProperty("format")) {
        info[k].format =
          typeof fmt.format === "function" ? fmt.format : FormatterRegistry.get(fmt.format as any, info[k].nullAlias);
      } else {
