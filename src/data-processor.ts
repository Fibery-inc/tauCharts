...
import { ChartDimensionsMap, Dimension } from "./definitions";
...
var DataProcessor = {
  ...
  sortByDim: function (data: any[], dimName: string, dimInfo: Dimension) {
    ...
    var interceptor = ["period", "time"].indexOf(dimInfo.scale) >= 0 ? (x) => new Date(x) : (x) => x;
    ...
    if (dimInfo.type === "measure" || dimInfo.scale === "period") {
      ...
      var hashOrder = dimInfo.order.reduce((memo, x, i) => {
        memo[x] = i;
        return memo;
      }, {});
