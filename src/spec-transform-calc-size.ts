...
type FitModelStrategy = (
  srcSize: Size,
  calcSize: (axis: "x" | "y", unit: Unit, stepStrategy: StepSizeStrategy, frame?: DataFrameObject) => number,
  specRef: GPLSpec,
...
var fitModelStrategies: FitModelStrategies = {
  "entire-view"(srcSize, calcSize, specRef, tryOptimizeSpec) {
    ...
    var shouldHideXAxis =
      (g.paddingNoTicks && srcSize.height - ticksBPad < specRef.settings.minChartHeight) ||
    ...
    var shouldHideYAxis =
      (g.paddingNoTicks && srcSize.width - ticksLPad < specRef.settings.minChartWidth) ||
      yFacetCount * specRef.settings.minFacetHeight + ticksBPad > srcSize.height ||
      xFacetCount * specRef.settings.minFacetWidth + ticksLPad > srcSize.width;
    if (shouldHideXAxis) {
...
export class SpecTransformCalcSize implements SpecTransformer {
  ...
  transform(chart: Plot) {
    ...
    var calcSizeRecursively = (
      ...
      if (
        firstUnit.type === "ELEMENT.INTERVAL" &&
        ...
        const distributeByColor =
          xScale.discrete &&
          (firstUnit.guide.enableColorToBarPosition == null
            ? !firstUnit.stack
        ...
        var rowsSizes = Object.keys(rows).map((kRow) => {
          return rows[kRow]
    ...
    var prettifySize = (srcSize: Size, newSize: Size, rScroll: number) => {
      ...
      var recommendedWidth =
        newSize.width > srcSize.width && newSize.width <= srcSize.width * rScroll ? srcSize.width : newSize.width;
      var recommendedHeight =
        newSize.height > srcSize.height && newSize.height <= srcSize.height * rScroll ? srcSize.height : newSize.height;
