...
export function traverseJSON(
  ...
  (rootRef[byProperty] || []).forEach((unit) =>
    traverseJSON(unit, byProperty, fnSelectorPredicates, funcTransformRules)
  );
...
export function splitEvenly(domain: [number, number], parts: number): number[] {
  ...
  var chunks = parts >= 2 ? range(parts - 2).map((n) => min + segment * (n + 1)) : [];
...
export function xml(tag: string) {
  ...
  var attrsString = Object.keys(attrs)
    .map(function (key) {
      return ` ${key}="${attrs[key]}"`;
    })
    .join("");
  if (attrsString.length > XML_ATTR_WRAP) {
    attrsString = Object.keys(attrs)
      .map(function (key) {
  ...
  const childrenString = hasSingleTextChild
    ...
    : "\n" +
      children
        ...
        .join("\n") +
      "\n";
  const tagEnding = isVoidTag ? "/>" : `>${childrenString}</${tag}>`;
...
const chartElements = [GenericCartesian];
