...
var d3_createPathTween = (attr, pathStringBuilder, pointConvertors, idGetter, interpolationType = "linear") => {
  ...
  return function (data) {
    ...
    const frames = pointConvertors.map((convertor, i) => {
      ...
      const interpolateLine = getLineInterpolator(interpolationType) || getLineInterpolator("linear");
      const pointsTo = interpolateLine(points);
      const pointsFrom = this[pointStore][i];

      const interpolate = interpolatePathPoints(pointsFrom, pointsTo, getInterpolatorSplineType(interpolationType));
