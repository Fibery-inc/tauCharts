import { getBezierPoint as bezierPt, splitCubicSegment as split } from "../bezier";
...
function getCirclesCurveTangents(a: Point, ca: Point, cb: Point, b: Point) {
  ...
  var getTangentsVectors = (isEnd) => {
    ...
    var lpoints = [mtan.left[0], bezierPt(0.5, mtan.left[1], ntan.left[0]), ntan.left[1]];
    var rpoints = [mtan.right[0], bezierPt(0.5, mtan.right[1], ntan.right[0]), ntan.right[1]];
