...
function getLinearInterpolator(pointsFrom: XPoint[], pointsTo: XPoint[]): (t: number) => XPoint[] {
  ...
  var remainingIds = idsFrom.filter((id) => idsTo.indexOf(id) >= 0);
  ...
  if (
    ...
    ((pointsTo.length > 0 && remainingIds.length === 0) || (pointsFrom.length > 0 && remainingIds.length === 0))
  ) {
    interpolators.push(
      getNonRemainingPathInterpolator({
        isCubic: false,
  ...
  remainingIds.forEach((id, i) => {
    ...
    if (i > 0) {
      ...
      if (indexFrom - prevIndexFrom > 3 || indexTo - prevIndexTo > 3) {
        ...
        interpolators.push(
          getControlsBetweenRemainingInterpolator({
            polylineFrom: pointsFrom.slice(prevIndexFrom, indexFrom + 1),
            polylineTo: pointsTo.slice(prevIndexTo, indexTo + 1),
          })
        );
    ...
    if (t > 0) {
      ...
      for (i = 1; i < distance.length; i++) {
        if (passedDistance <= distance[i]) {
          q = Math.min(
            1,
            (i - 1 + (passedDistance - distance[i - 1]) / (distance[i] - distance[i - 1])) / (line.length - 1)
          );
          break;
        }
      }
    ...
    if (t > 0) {
      ...
      for (i = 1; i < distance.length; i++) {
        if (passedDistance <= distance[i]) {
          q = Math.min(
            1,
            (i - 1 + (passedDistance - distance[i - 1]) / (distance[i] - distance[i - 1])) / (pointsCount - 1)
          );
          break;
        }
      }
...
function fillSmallerPolyline({ smallerPolyline, biggerPolyline, decreasing }) {
  ...
  var segmentsPointsCount = utils
    .range(smallerSegCount)
...
function fillSmallerCubicLine({ smallerPolyline, biggerPolyline, decreasing }) {
  ...
  segmentsPointsCount.forEach((segPtCount) => {
    if (segPtCount > 2) {
      ...
      utils.range(segPtCount - 2).forEach((i) => (spl[(i + 1) * 3].id = biggerPolyline[result.length - 1 + i * 3].id));
      if (decreasing) {
        spl.forEach((p, i) => {
...
function getScaleDiffFn(points1, points2) {
  ...
  var propDiffs = {};
  var createPropDiffFn = (a0, b0, a, b) => (c0) => b + ((c0 - b0) * (b - a)) / (b0 - a0);
