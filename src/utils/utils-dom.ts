...
export function sortChildren(parent: Element, sorter: (a: Element, b: Element) => number) {
  if (parent.childElementCount > 0) {
    ...
    const unsorted = Array.prototype.filter.call(parent.childNodes, (el) => el.nodeType === Node.ELEMENT_NODE);
    ...
    const createIterations = (forward) => {
      const iterations = groups
        ...
        .sort(
          utils.createMultiSorter<{ elements; to }>(
    ...
    const iterations = forwardIterations.length < backwardIterations.length ? forwardIterations : backwardIterations;

    // Finally sort DOM nodes
    const mirror = unsortedGroups.map((g) => g.elements);
    iterations.forEach((g) => {
