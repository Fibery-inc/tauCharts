...
export function raiseElements(
  ...
  const highlighted: d3.Selection<Element, any, Element, any> = <any>container.selectAll(selector).filter(filter);
  if (highlighted.empty()) {
    return;
  }
  const untargeted = d3
    .select((<any>highlighted.node()).parentNode)
    .selectAll(selector)
    .filter((d) => !filter(d))
    .nodes();
  const lastUntargeted: Element = <any>untargeted[untargeted.length - 1];
  if (lastUntargeted) {
    const untargetedIndex = Array.prototype.indexOf.call(lastUntargeted.parentNode.childNodes, lastUntargeted);
