...
import { d3_setClasses as setClasses, d3_transition as transition } from "../utils/d3-decorators";
...
export class Plot extends Emitter {
  ...
  constructor(config: ChartConfig) {
    ...
    this.onUnitsStructureExpandedTransformers = [SpecTransformExtractAxes, SpecTransformCalcSize];
  ...
  setupConfigSettings(config: ChartConfig) {
    ...
    config.settings = Plot.setupSettings(utils.defaults(config.settings || {}, this._dataRefs));
    return config;
  }
  ...
  setupConfig(config: ChartConfig) {
    ...
    var resConfig: ChartConfig = utils.defaults(config, {
      spec: {},
      data: [],
      plugins: [],
      settings: {},
    ...
    if (resConfig.settings.excludeNull) {
      this.addFilter({
        tag: "default",
        src: "/",
        predicate: DataProcessor.excludeNullValues(resConfig.spec.dimensions, (item) =>
  ...
  static setupPeriodData(spec: GPLSpec) {
    ...
    var scales = Object.keys(spec.scales).map((s) => spec.scales[s]);
  ...
  static setupSettings(configSettings: ChartSettings) {
    ...
    var localSettings = Object.keys(globalSettings).reduce((memo, k) => {
      memo[k] = typeof globalSettings[k] === "function" ? globalSettings[k] : utils.clone(globalSettings[k]);
      return memo;
    }, {} as ChartSettings);
  ...
  onUnitDraw(unitNode: GrammarElement) {
    ...
    ["click", "mouseover", "mouseout"].forEach((eventName) =>
      unitNode.on(eventName, (sender, e) => {
        this.fire(`element${eventName}`, <PointerEventArgs>{
          element: sender,
          data: e.data,
          event: e.event,
        });
  ...
  onUnitsStructureExpanded(specRef: GPLSpec) {
    this.onUnitsStructureExpandedTransformers.forEach((TClass) => new TClass(specRef).transform(this));
  ...
  _handlePointerEvent(event: MouseEvent) {
    ...
    if (nonEmpty.length > 0) {
      const largerDistIndex = nonEmpty.findIndex(
        (d) =>
  ...
  _initPointerEvents() {
    ...
    svg.on(
      "mouseleave",
      wrapEventHandler((event) => {
        if (window.getComputedStyle(this._svg).pointerEvents !== "none") {
          this.select(() => true).forEach((unit) =>
            unit.fire("data-hover", { event, data: null, node: null, unit: null })
          );
        }
      })
    );
  ...
  _createLiveSpec() {
    ...
    this._liveSpec = this.transformers.reduce(
      (memo, TransformClass) => new TransformClass(memo).transform(this),
      this._liveSpec
    );
  ...
  _experimentalSetupAnimationSpeed(spec: GPLSpec) {
    ...
    (<any>spec.settings).initialAnimationSpeed =
      (<any>spec.settings).initialAnimationSpeed || spec.settings.animationSpeed;
  ...
  _renderRoot({ scenario, newSize }: { scenario: GrammarElement[]; newSize: Size }) {
    ...
    var roots = (svg.selectAll("g.frame-root") as d3.Selection<SVGElement, string, SVGSVGElement, any>).data(
      [frameRootId],
      (x) => x
    );
    ...
    roots
      ...
      .call((selection) => {
        ...
        transition(selection, this.configGPL.settings.animationSpeed, "frameRootToggle").attr("opacity", 1);
      });
    roots.exit().call((selection) => {
      selection.classed("tau-active", false);
      transition(selection, this.configGPL.settings.animationSpeed, "frameRootToggle").attr("opacity", 1e-6).remove();
    });
  }
  ...
  _createProgressBar() {
    ...
    var progressValue = progressBar.append("div").classed(`${CSS_PREFIX}progress__value`, true).style("width", 0);
  ...
  getSourceFiltersIterator(rejectFiltersPredicate: (filter: Filter) => boolean) {
    var filters = utils
      .flatten(Object.keys(this._filtersStore.filters).map((key) => this._filtersStore.filters[key]))
  ...
  getDataSources(param: ExcludeFilter = {}) {
    var excludeFiltersByTagAndSource = (k: string) => (f: Filter) =>
      (param.excludeFilter && param.excludeFilter.indexOf(f.tag) !== -1) || f.src !== k;

    var chartDataModel = this._chartDataModel(this._originData);

    return Object.keys(chartDataModel)
      .filter((k) => k !== "?")
      .reduce(
        (memo, k) => {
  ...
  isEmptySources(sources: DataSources) {
    return !Object.keys(sources)
      .filter((k) => k !== "?")
      .filter((k) => sources[k].data.length > 0).length;
  }
  ...
  _displayTimeoutWarning({ proceed, cancel, timeout }: { proceed: () => void; cancel: () => void; timeout: number }) {
    ...
    this._layout.content.insertAdjacentHTML(
      "beforeend",
      `
    ...
    this._layout.content.querySelector(`.${CSS_PREFIX}rendering-timeout-continue-btn`).addEventListener("click", () => {
      this._clearTimeoutWarning();
      proceed.call(this);
    });
    this._layout.content.querySelector(`.${CSS_PREFIX}rendering-timeout-cancel-btn`).addEventListener("click", () => {
      this._clearTimeoutWarning();
      cancel.call(this);
    });
  }
