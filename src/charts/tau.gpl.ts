...
const MixinModel: new (prev: GrammarModel) => GrammarModel = <any>function (prev: GrammarModel) {
  Object.keys(prev).forEach((k) => (this[k] = prev[k]));
};
...
export class GPL extends Emitter {
  ...
  constructor(
    ...
    this.transformations = Object.assign(config.transformations || {}, {
      where(data, tuple) {
  ...
  getDrawScenarioQueue(root: GrammarElement) {
    ...
    var scaleInfoQueue = this._flattenDrawScenario(root, (parentInstance, unit, rootFrame) => {
      ...
      const instance = this.unitSet.create(
        unit.type,
        Object.assign({}, unit, { options: parentInstance.allocateRect(rootFrame.key) })
      );
    ...
    var createScales = () => {
      Object.keys(this.scales).forEach((k) => this.scalesHub.createScaleInfo(this.scales[k]).commit());
    };
  ...
  _expandUnitsStructure(root: Unit, parentPipe: DataFilter[] = []) {
    ...
    root.frames.forEach((f) => f.units.forEach((unit) => this._expandUnitsStructure(unit, f.pipe)));
