...
interface NormalizeAxisStrategies {
  [status: string]: (axis: string[], config: ValidatedConfig, guide: UnitGuide[]) => string[];
}
...
function normalizeConfig(config: ChartConfig) {
  ...
  return Object.assign({}, config, { x, y, guide }, encodings);
}
...
function transformConfig(type: string, config: ChartConfig) {
  ...
  for (var i = maxDepth; i > 0; i--) {
    ...
    if (i === maxDepth) {
      ...
      spec.guide = utils.defaults(currentGuide, {
        x: { label: currentX },
        y: { label: currentY },
      });
    } else {
      spec = {
        ...
        y: convertAxis(currentY),
        unit: [spec],
        guide: utils.defaults(currentGuide, {
          x: { label: currentX },
          y: { label: currentY },
        }),
      };
