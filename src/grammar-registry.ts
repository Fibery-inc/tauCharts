...
import { GrammarModel, GrammarRule, ScaleConfig, ScaleFunction } from "./definitions";
...
GrammarRegistry.reg("identity", () => {
  ...
  .reg("flip", (model) => {
    ...
    const min = valsScale.discrete ? ys[0] : Math.max(0, Math.min(...ys)); // NOTE: max also can be below 0
  ...
  .reg("positioningByColor", (model) => {
    var method: GrammarRule = model.scaleX.discrete
      ? (model) => {
          ...
          const xColors = dataSource.reduce((map, row) => {
  ...
  .reg("stack", (model) => {
    ...
    if (yScale.discrete || yScale.domain().some((x) => typeof x !== "number")) {
      throw new Error(`Stacked field [${yScale.dim}] should be a number`, errorCodes.STACKED_FIELD_NOT_NUMBER, {
        field: yScale.dim,
      });
    }
    ...
    const nextData = Object.keys(groups)
      .sort(
        model.flip || (!model.flip && model.obsoleteVerticalStackOrder)
  ...
  .reg("avoidScalesOverflow", (model, { sizeDirection }) => {
    ...
    const shouldIgnoreScale = (scale, direction) =>
      !scale || scale.discrete || scale.scaleType === "logarithmic" || sizeDirection.indexOf(direction) < 0;
  ...
  .reg("fillGaps", (model, { isStack, xPeriod, utc }) => {
    ...
    const fibers = Object.keys(groups)
    ...
    const asc = (a, b) => a - b;
    const getUsualXs = () =>
      utils.unique(fibers.reduce((memo, fib) => memo.concat(fib.map((row) => row[dx])), [])).sort(asc);
    const getPeriodicXs = () => {
      ...
      const ticks = UnitDomainPeriodGenerator.generate(domain[0], domain[1], xPeriod, { utc }).filter(
