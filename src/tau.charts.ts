...
api.scalesRegistry
  .reg("identity", IdentityScale, (config, settings) =>
    utils.defaults(config, {
      references: settings.references,
      refCounter: settings.refCounter,
    })
  )
  .reg("color", ColorScale, (config, settings) =>
    utils.defaults(config, {
  ...
  .reg("period", PeriodScale, (config, settings) =>
    utils.defaults(config, {
      utcTime: settings.utcTime,
    })
  )
  .reg("time", TimeScale, (config, settings) =>
    utils.defaults(config, {
      utcTime: settings.utcTime,
    })
  )
...
var commonRules = [(config) => (!config.data ? ["[data] must be specified"] : ([] as string | string[]))];
...
export { GPL, Plot, Chart, api, version };
