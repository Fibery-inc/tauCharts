...
function createAxis(config: AxisConfig) {
  ...
  const { ticksCount, tickFormat, tickSize, tickPadding, gridOnly } = defaults(config, {
    tickSize: 6,
    tickPadding: 3,
    gridOnly: false,
  });
  ...
  return (context: d3Selection | d3Transition) => {
    ...
    function drawDomain() {
      ...
      take(selection.selectAll(".domain").data(domainLineData))
        .then((path) => {
          if (transition) {
            path.exit().transition(transition).attr("opacity", 0).remove();
          }
          return path.merge(
            path.enter().insert("path", ".tick").attr("class", "domain").attr("opacity", 1).attr("stroke", "#000")
          );
        })
        .then((path) => {
          return transition ? path.transition(transition) : path;
        })
        .then((path) => {
          path.attr(
            "d",
    ...
    function createTicks(): TickDataBinding {
      return take((selection.selectAll(".tick") as d3Selection).data(values, (x) => String(scale(x))).order())
        .then((tick) => {
        ...
        .then((result) => {
          if (isLinearScale) {
            ...
            result.tick.classed("zero-tick", (d) => {
    ...
    function drawLines(ticks: TickDataBinding) {
      ...
      take(ticks)
        .then(({ tick, tickEnter }) => {
          ...
          const lineEnter = tickEnter.append("line").attr("stroke", "#000").attr(`${y}2`, ly);

          if (isOrdinalScale) {
            lineEnter.attr(`${x}1`, lx).attr(`${x}2`, lx);
          }
    ...
    function drawExtraOrdinalLine() {
      ...
      take(selection.selectAll(".extra-tick-line").data([null]))
        .then((extra) => {
          return extra.merge(
            extra.enter().insert("line", ".tick").attr("class", "extra-tick-line").attr("stroke", "#000")
          );
        })
        .then((extra) => {
          return transition ? extra.transition(transition) : extra;
        })
        .then((extra) => {
          extra
            .attr(`${x}1`, range0)
    ...
    function drawText(ticks: TickDataBinding) {
      ...
      function fixTextPosForVerticalFacets(selection) {
        if (scaleGuide.facetAxis) {
          return selection.attr("dx", -config.position[0] + 18).attr("dy", 16);
        }
      }
      ...
      take(ticks)
        .then(({ tick, tickEnter }) => {
          ...
          const textEnter = tickEnter.append("text").attr("fill", "#000").attr(y, ty).attr("dy", tdy);
        ...
        .then((text) => {
          text.text(format).attr("text-anchor", textAnchor);

          if (isHorizontal === false && scaleGuide.facetAxis === true) {
            const facetShift = utilsDraw.parseTransformTranslate(selection.node().parentNode.getAttribute("transform"));
    ...
    function rotateText(text: d3Selection | d3Transition) {
      ...
      text.attr("transform", utilsDraw.rotate(angle));
      ...
      if (Math.abs(angle / 90) % 2 > 0) {
        ...
        let dy = k * (orient === Orient.top || orient === Orient.bottom ? (sign < 0 ? 0 : 0.71) : 0.32);
    ...
    function drawAxisLabel() {
      ...
      let tspans = labelTextNode
        ...
        .attr("opacity", epsilon)
        .attr("class", (d, i) =>
          i % 2 ? `label-token-delimiter label-token-delimiter-${i}` : `label-token label-token-${i}`
        )
        .text((d) => d);

      let tspansExit = tspans.exit();

      if (transition) {
        tspans = tspans.transition(transition);

        tspansExit = tspansExit.transition(transition).attr("opacity", epsilon);
      }
    ...
    if (isOrdinalScale && gridOnly) {
      // Todo: Explicitly determine if grid
