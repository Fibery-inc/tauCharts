...
import { d3Selection, GrammarModel, ScaleGuide } from "../../definitions";
...
var intersect = (x1, x2, x3, x4, y1, y2, y3, y4) => utilsDraw.isIntersect(x1, y1, x2, y2, x3, y3, x4, y4);
...
export class LayerLabels {
  ...
  constructor(
    ...
    this.w = width;
    this.h = height;
    this.guide = utils.defaults(labelGuide || {}, {
  ...
  draw(fibers: any[][]) {
    ...
    var parallel = fibers.reduce(
      (memo, f) => {
        ...
        memo.edges = memo.edges.concat(
          readBy3(absFiber, (prev, curr, next) => {
    ...
    parallel.text = parallel.text.filter((r) => r.label).map((r, i) => Object.assign(r, { i: i }));

    var positions = lineBreakAvailable
      ? ["auto:hide-on-label-label-overlap", "auto:adjust-on-multiline-label-overflow"]
      : this.guide.position;
    ...
    if (guide.hideEqualLabels) {
      labels
        .filter((d) => !d.hide)
        .filter((d, i, visibleLabels) => i < visibleLabels.length - 1 && d.label === visibleLabels[i + 1].label)
        .forEach((d) => (d.hide = true));
    }

    var text = this.container.selectAll(".i-role-label").data(labels.map((r) => r.data));
    text.exit().remove();
    text.call(update);
  ...
  applyFixedPositionRules(guide, args, seed, lineBreakAvailable, flip) {
    var fixedPosition = guide.position.filter((token) => token.indexOf("auto:") === -1);
  ...
  hideOnLabelLabelOverlap(data: TextInfo[]) {
    ...
    var cross = (a: TextInfo, b: TextInfo) => {
      ...
      if (x_overlap * y_overlap > 0) {
        ...
        let r = collisionSolveStrategies[`${p[0].extr}/${p[1].extr}`](p[0], p[1]) < 0 ? p[0] : p[1];
        r.hide = true;
      }
    };
