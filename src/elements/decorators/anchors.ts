...
import { d3_animationInterceptor as createUpdateFunc, d3_setAttrs as attrs } from "../../utils/d3-decorators";
import * as utilsDom from "../../utils/utils-dom";
import { isNonSyntheticRecord } from "../../utils/utils-grammar";
import { d3Selection, GrammarElement, GrammarModel, UnitGuide } from "../../definitions";
...
const anchorShapes: ShapesAttrsDictionary = {
  circle: {
    ...
    getInitialAttrs: function (node, model) {
      ...
      const screenModel = node.screenModel;
      return {
        r: guide.showAnchors === "hover" ? 0 : (d) => screenModel.size(d) / 2,
        cx: (d) => model.x(d),
        cy: (d) => model.y(d),
      };
    },
    ...
    getHighlightAttrs: function (node, model, filter) {
      ...
      return {
        d: (d) => {
          ...
          const r = showOnHover
            ? filter(d)
              ? Math.max(rmin, screenModel.size(d) / 2)
              : 0
            : filter(d)
