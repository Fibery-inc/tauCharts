...
const cutString = (str: string, index: number) =>
  index === 0 ? "" : str.slice(0, index).replace(/\.+$/g, "") + "\u2026";
...
LayerLabelsRules.regRule("l", alignByX(["l", -1, null]))
    ...
    return {
      ...
      dy: (row) => {
        ...
        if (prev.angle(row) !== 0) {
          ...
          return available < reserved ? (available * prevDy) / reserved : prevDy;
        }
  ...
  .regRule("cut-outer-label-vertical", (prev) => {
    return {
      h: (row, args) => {
        ...
        if (Math.abs(prev.angle(row)) > 0) {
          const available =
            prev.model.y0(row) < prev.model.yi(row) ? args.maxHeight - prev.model.yi(row) : prev.model.yi(row);
          return available < reserved ? available : reserved;
        }
  ...
  .regRule("cut-outer-label-horizontal", (prev, args) => {
    return {
      dx: (row) => {
        const required = prev.w(row);
        const available =
          prev.model.y0(row) < prev.model.yi(row) ? args.maxWidth - prev.model.yi(row) : prev.model.yi(row);
        const prevDx = prev.dx(row);
        return available < required ? (available * prevDx) / required : prevDx;
      },
