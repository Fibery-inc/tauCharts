...
export class Parallel extends Element {
  ...
  defineGrammarModel(fnCreateScale) {
    ...
    this.columnsScalesMap = cfg.columns.reduce((memo, xi) => {
    ...
    var colsMap = cfg.columns.reduce((memo, p, i) => {
      memo[p] = i * step;
      return memo;
    }, {});
  ...
  drawFrames(frames) {
    ...
    var updateCellLayers = (cellId, cell, frame) => {
      var layers = cell.selectAll(`.layer_${cellId}`).data(frame.units, (unit) => unit.uid);
      layers.exit().remove();
      layers
        .enter()
        .append("g")
    ...
    var grid = this._fnDrawGrid(
      ...
      cfg,
      options.frameId,
      Object.keys(this.columnsScalesMap)
        .reduce((memo, k) => memo.concat([this.columnsScalesMap[k].getHash()]), [])
        .join("_")
    );

    var frms = grid.selectAll(`.parent-frame-${options.frameId}`).data(frames, (f) => f.hash());
    frms.exit().remove();
    frms
      .enter()
      .append("g")
  ...
  _fnDrawGrid(container, config, frameId, uniqueHash) {
    var grid = container.selectAll(`.grid_${frameId}`).data([uniqueHash], (x) => x);
    grid.exit().remove();
    const merged = grid
      .enter()
      .append("g")
  ...
  _fnDrawColumns(grid, config) {
    ...
    var cols = grid.selectAll(".column").data(config.columns, (x) => x);
    cols.exit().remove();
    const merged = cols
      .enter()
      .append("g")
  ...
  _enableBrushing(cols) {
    ...
    var onBrushEventHandler = () => {
      var targetKey = Object.keys(columnsBrushes).find((k) => columnsBrushes[k] === d3Selection.event.target);
