...
import { d3_animationInterceptor, d3_setClasses as classes } from "../utils/d3-decorators";
...
const Interval = {
  init(xConfig) {
    ...
    config.guide = utils.defaults(config.guide, {
      ...
      enableColorToBarPosition:
        config.guide.enableColorToBarPosition != null ? config.guide.enableColorToBarPosition : !config.stack,
    });

    config.guide.size = utils.defaults(config.guide.size || {}, {
    ...
    config.guide.label = utils.defaults(config.guide.label || {}, {
      position: config.flip
        ? config.stack
          ...
          : ["outside-then-inside-horizontal", "auto:hide-on-label-label-overlap"]
        : config.stack
        ? [
        ...
        : ["rotate-on-size-overflow", "outside-then-inside-vertical", "auto:hide-on-label-label-overlap"],
    });
  ...
  draw() {
    ...
    const data = fibers.reduce((arr, f) => arr.concat(f), []);

    const barClass = d3Attrs.class;
    const updateAttrs = utils.omit(d3Attrs, "class");
    const bars = options.container.selectAll(".bar").data(data, screenModel.id);
    bars
      .exit()
    ...
    const merged = bars
      .enter()
      .append("rect")
      .call(createUpdateFunc(config.guide.animationSpeed, { [barY]: screenModel[`${barY}0`], [barH]: 0 }, updateAttrs))
      .merge(bars)
      .attr("class", barClass)
      .attr("data-zero", screenModel[`${barY}0`]);

    node.subscribe(
      new LayerLabels(screenModel.model, screenModel.model.flip, config.guide.label, options).draw(fibers)
    );
    const sortByOrder = (() => {
  ...
  _getBoundsInfo(bars) {
    ...
    const ticks = utils.unique(items.map(flip ? (item) => item.cy : (item) => item.cx)).sort((a, b) => a - b);
  ...
  highlight(filter) {
    ...
    container.selectAll(".bar").call(classes(classed));

    container.selectAll(".i-role-label").call(classes(classed));

    this._sortElements((a, b) => filter(d3Data(a)) - filter(d3Data(b)), this._typeSorter, this._barsSorter);
  },
};
