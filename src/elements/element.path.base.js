...
const BasePath = {
  ...
  draw() {
    ...
    const updateGroupContainer = function (selection) {
      ...
      points.exit().call(createUpdateFunc(guide.animationSpeed, null, { r: 0 }, (node) => d3.select(node).remove()));
      ...
      const updatePath = (selection) => {
        if (config.guide.animationSpeed > 0 && !document.hidden) {
          ...
          transition(selection, config.guide.animationSpeed, "pathTransition").attrTween(
            model.pathTween.attr,
            model.pathTween.fn
          );
        } else {
          selection.attr(model.pathTween.attr, function (d) {
    ...
    const frameBinding = frameSelection.data(fullFibers, getDataSetId);
  ...
  _getBoundsInfo(dots) {
    ...
    const ticks = utils.unique(items.map(flip ? (item) => item.y : (item) => item.x)).sort((a, b) => a - b);
  ...
  getClosestElement(cursorX, cursorY) {
    ...
    if (
      cursorX < bounds.left + translate.x - maxHighlightDistance ||
  ...
  highlight(filter) {
    ...
    const targetFibers = paths.data().filter((fiber) => {
    ...
    container.selectAll(".i-role-dot").call(classes(classed));
