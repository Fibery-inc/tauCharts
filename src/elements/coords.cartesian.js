...
import { d3_transition as transition, d3_selectAllImmediate as selectAllImmediate } from "../utils/d3-decorators";
...
export class Cartesian extends Element {
  constructor(config) {
    ...
    this.config.guide.x = utils.defaults(this.config.guide.x, {
      cssClass: "x axis",
      textAnchor: "middle",
      padding: 10,
      hide: false,
    ...
    this.config.guide.x.label = utils.defaults(this.config.guide.x.label, {
      text: "X",
      rotate: 0,
      padding: 40,
      textAnchor: "middle",
    ...
    this.config.guide.y = utils.defaults(this.config.guide.y, {
      cssClass: "y axis",
      textAnchor: "start",
      padding: 10,
      hide: false,
    ...
    this.config.guide.y.label = utils.defaults(this.config.guide.y.label, {
      text: "Y",
      rotate: -90,
      padding: 20,
      textAnchor: "middle",
  ...
  defineGrammarModel(fnCreateScale) {
    ...
    this.yScale = fnCreateScale("pos", this.config.y, (scaleConfig) =>
      ["ordinal", "period"].indexOf(scaleConfig.type) >= 0 ? [0, h] : [h, 0]
    );
  ...
  drawFrames(frames) {
    ...
    var xcells = selectAllImmediate(grid, ".cell").data(xdata, (x) => x);
    xcells
      .enter()
      .append("g")
    ...
    transition(xcells, this.config.guide.animationSpeed).attr("opacity", 1);
  ...
  _removeDimAxis(container, scale) {
    var axis = selectAllImmediate(container, this._getAxisSelector(scale)).classed("tau-active", false);
    transition(axis, this.config.guide.animationSpeed, "axisTransition").attr("opacity", 1e-6).remove();
  }
      ...
      .call((selection) => {
        ...
        if (linesOptions.length > 0) {
          ...
          if (linesOptions.indexOf("x") > -1) {
            ...
            var xGridLinesTrans = transition(xGridLines, animationSpeed).call(xGridAxis);
          }
          ...
          if (linesOptions.indexOf("y") > -1) {
            ...
            var yGridLinesTrans = transition(yGridLines, animationSpeed).call(yGridAxis);
          }
        }
      });
