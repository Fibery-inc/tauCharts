...
const Area = {
  ...
  _getBoundsInfo(this: GrammarElement, dots: Element[]) {
    ...
    const split = (values: number[]): TreeNode => {
      ...
      const midIndex = values.length % 2 === 0 ? values.length / 2 : (values.length - 1) / 2;
  ...
  getClosestElement(cursorX: number, cursorY: number) {
    ...
    var kx =
      closestSpan.end === closestSpan.start ? 0 : (cursor - closestSpan.start) / (closestSpan.end - closestSpan.start);
    if (kx < 0) {
      kx = 0;
    }
    if (kx > 1) {
    ...
    const cursorOverItems = interpolated.filter((d) => cy >= d.y && cy <= d.y0);
    const bestMatchItems = cursorOverItems.length > 0 ? cursorOverItems : interpolated;

    const bestElements = bestMatchItems
      .map((d) => d.el)
      .map((el) => {
