...
export class GeoMap extends Element {
  ...
  _calcLabels(topoJSONData, reverseContours, path) {
    ...
    reverseContours.forEach((c) => {
      ...
      d3Labeler().label(labels).anchor(anchors).width(innerW).height(innerH).start(iterationsCount);
  ...
  _drawMap(frames, topoJSONData) {
    ...
    var xmap = node.selectAll(".map-container").data([`${innerW}${innerH}${center}${contours.join("-")}`], (x) => x);
    xmap.exit().remove();
    const merged = xmap
      .enter()
      .append("g")
      .call(function (selection) {
        ...
        reverseContours.forEach((c, i) => {
          ...
          node
            ...
            .call(function (selection) {
              ...
              cont.append("title").text((d) => (d.properties || {}).name);
        ...
        if (topoJSONData.objects.hasOwnProperty("places")) {
          ...
          var labels = placesFeature.features.map((d) => {
          ...
          var anchors = placesFeature.features.map((d) => {
            var coords = d3Projection(d.geometry.coordinates);
            return {
              x: coords[0],
              y: coords[1],
          ...
          d3Labeler().label(labels).anchor(anchors).width(innerW).height(innerH).start(100);
    ...
    this.groupByCode = frames.reduce((groups, f) => {
      return f.part().reduce((memo, rec) => {
    ...
    merged
      .selectAll(`.map-contour-${contourToFill}`)
      .data(topojson.feature(topoJSONData, topoJSONData.objects[contourToFill]).features)
      .call(function (selection) {
        selection.classed("map-contour", true).attr("fill", (d) => {
          var row = toData(d);
    ...
    var updateGroups = function (selection) {
      selection
        .attr("class", (f) => `frame frame-${f.hash}`)
        .call(function (selection) {
    ...
    var frameGroups = merged.selectAll(".frame").data(frames.map(mapper), (f) => f.hash);
    frameGroups.exit().remove();
    frameGroups.enter().append("g").merge(frameGroups).call(updateGroups);
