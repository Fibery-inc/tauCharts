...
import { d3_setAttrs as attrs, d3_setClasses as classes, d3_transition } from "../utils/d3-decorators";
import { d3Selection, GrammarElement, GrammarModel, GrammarRule } from "../definitions";
...
const Point: PointClass = {
  init(xConfig) {
    ...
    config.guide = utils.defaults(config.guide || {}, {
      animationSpeed: 0,
    ...
    config.guide.label = utils.defaults(config.guide.label || {}, {
      position: [
    ...
    config.transformRules = [
      (prevModel: GrammarModel) => {
        const bestBaseScale = [prevModel.scaleX, prevModel.scaleY].sort((a, b) => {
        ...
        return isHorizontal ? GrammarRegistry.get("flip")(prevModel) : GrammarRegistry.get("identity")(prevModel);
      },
    ...
    config.adjustRules = [
      ...
      (prevModel: GrammarModel, args) => {
        ...
        const sizeCfg = utils.defaults(config.guide.size, {
          defMinSize: 10,
  ...
  draw(this: PointInstance) {
    ...
    const updateGroups = function (g: d3Selection) {
      g.attr("class", "frame").call(function (c) {
        var dots = c.selectAll("circle").data((fiber) => fiber, screenModel.id);

        var dotsEnter = dots.enter().append("circle").call(attrs(circleTransAttrs));

        var dotsMerge = dotsEnter.merge(dots).call(attrs(circleAttrs));

        transition(dotsMerge).call(attrs(circleTransAttrs));
    ...
    const frameGroups = options.container.selectAll(".frame").data(fibers, (f) => screenModel.group(f[0]));

    const merged = frameGroups.enter().append("g").attr("opacity", 0).merge(frameGroups).call(updateGroups);
  ...
  getClosestElement(this: PointInstance, _cursorX, _cursorY) {
    ...
    if (
      cursorX < bounds.left - maxHighlightDistance ||
    ...
    const items = (tree.find(cursorX, cursorY) || [])
      .map((item) => {
        const distance = Math.sqrt(Math.pow(cursorX - item.x, 2) + Math.pow(cursorY - item.y, 2));
    ...
    const largerDistIndex = items.findIndex(
      (d) => d.distance !== items[0].distance || d.secondaryDistance !== items[0].secondaryDistance
    );
  ...
  highlight(this: PointClass, filter: HighlightFilter) {
    ...
    container.selectAll(".dot").call(classes(classed));

    container.selectAll(".i-role-label").call(classes(classed));
  ...
  _sortElements(this: PointInstance, filter: HighlightFilter) {
    ...
    container.selectAll(".frame").each(function (d: any[]) {
