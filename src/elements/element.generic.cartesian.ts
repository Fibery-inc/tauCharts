...
export abstract class GenericCartesian extends Element {
  ...
  defineGrammarModel(fnCreateScale: ScaleFactoryMethod): GrammarModel {
    ...
    this.regScale("x", fnCreateScale("pos", config.x, [0, config.options.width]))
      ...
      .regScale(
        "y",
        fnCreateScale("pos", config.y, (scaleConfig) =>
          ["ordinal", "period"].indexOf(scaleConfig.type) >= 0 ? [0, config.options.height] : [config.options.height, 0]
        )
      )
    ...
    const min = scaleY.discrete ? ys[0] : Math.max(0, Math.min(...ys)); // NOTE: max also can be below 0
  ...
  drawFrames() {
    ...
    var drawPart = function (that: d3Selection, id: string, props) {
      ...
      var part = that.selectAll(`.${id}`).data((row) => [row], self.screenModel.id);
      part.exit().call(createUpdateFunc(speed, null, { width: 0 }, (node) => d3.select(node).remove()));
    ...
    var updateGroups = function (selection) {
      selection.attr("class", `frame-id-${self.config.uid}`).call(function (selection) {
        var generic = selection.selectAll(".generic").data((fiber) => fiber, self.screenModel.id);
        generic.exit().remove();
