...
const Editor = createClass({
      ...
      <div>
            ...
            <div className="controls">
              <div className="controls-place">
                <NavButtons
                  ...
                  getConfigByNumber={this.getConfigByNumber}
                  maxConfig={this.props.configs.length}
                />
              </div>
        ...
        <div className="description">
          <div className="content-side">
            <p>
              <span className="head">{this.state.config.description[0]}</span>
              <b>{this.state.config.description[1]}</b>
            </p>
          </div>
        </div>
  ...
  getRandomConfig: function () {
    ...
    var measures = {
      OscarNominees: _.difference(
        keys["OscarNominees"],
        _.union(categorical["OscarNominees"], dates["OscarNominees"], ids["OscarNominees"])
      ),
      Comets: _.difference(keys["Comets"], _.union(categorical["Comets"], dates["Comets"], ids["Comets"])),
      WorldBank: _.difference(
        keys["WorldBank"],
        _.union(categorical["WorldBank"], dates["WorldBank"], ids["WorldBank"])
      ),
      EnglishPremierLeague: _.difference(
        keys["EnglishPremierLeague"],
        _.union(categorical["EnglishPremierLeague"], dates["EnglishPremierLeague"], ids["EnglishPremierLeague"])
      ),
    };
...
var SelectPropertyLink = createClass({
  render: function () {
    ...
    var apost =
      (this.props.type === "string" || this.props.type === "array") && this.props.name !== "data"
        ? ["'", "'"]
        : ["", ""];
...
var DropDownMenu = createClass({
  ...
  handleClick: function (event) {
    ...
    var result =
      checked.length > maxChecked ? checked.splice(checked.length - maxChecked, checked.length - 1) : checked;
  ...
  render: function () {
    ...
    if (type === "array") {
      links = value.map(function (link, i) {
        ...
        return <SelectPropertyLink key={i} value={link} type={type} name={name} isNotLast={isNotLast} />;
      });
      links.unshift("[");
      links.push("]");
    }
    ...
    if (name === "plugins") {
      return (
        <dl className={name}>
          <dt>{name}:</dt>
          <dd className={type}>
            <PluginsBlock value={value} options={options} updateConfig={this.props.updateConfig} />
          </dd>
        </dl>
      );
    }
    ...
    return (
      <dl className={name}>
        <dt>{name}:</dt>
        <dd className={type} onClick={this.handleClick}>
          {menu ? (
            <div>
              ...
              <DropDownMenu
                ...
                replaceDataset={this.props.replaceDataset}
...
var PluginLine = createClass({
  render: function () {
    ...
    return (
      <li className={isEnabled} onClick={this.handleClick}>
        tauCharts.api.plugins.get(<button className="btn-link">'{name}'</button>
        )(),
      </li>
    );
  },
    ...
    var plugins = pluginsList.map(function (plugin, i) {
      ...
      return (
        <PluginLine
          ...
          name={plugin}
          isEnabled={isEnabled}
          updateConfig={self.props.updateConfig}
...
const ChartConfig = createClass({
  ...
  render: function () {
    ...
    var fields = _.filter(_.keys(config), function (key) {
      ...
      return (
        <PropertyLine
          ...
          options={options}
          datasets={datasets}
          menuItem={self.state.menuItem}
          updateState={self.updateState}
          replaceDataset={self.props.replaceDataset}
          updateConfig={self.props.updateConfig}
        />
      );
    });
    ...
    return (
      <div>
        <p>
          <em>var</em> chart <em>= new</em> tauCharts.Chart(&#123;
        </p>
        {fields}
        <p>&#125;);</p>
        <p>
          chart.renderTo(<em>'#container'</em>);
        </p>
      </div>
    );
  },
  ...
  render: function () {
    return (
      <div className="navigator">
        <button className="prev-chart" onClick={this.prevChartClick} disabled={this.state.configNumber <= 0}>
          &nbsp;
        </button>
        <button
          className="next-chart"
          onClick={this.nextChartClick}
          disabled={this.state.configNumber >= this.props.maxConfig - 1}
        >
          <span>Next example</span>
        </button>
        <button className="lucky-chart" onClick={this.luckyClick}>
          &nbsp;
        </button>
      </div>
    );
