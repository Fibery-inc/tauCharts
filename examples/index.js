(function () {
  ...
  var PATHS = {
    ...
    "dev-quick-test/": fileRange("ex-", range(0, 31), range(40, 48), range(50, 54)),
    "datasets/": ["cars", "countries", "exoplanets", "data", "medals", "olympics", "tpStories"],
  };
  ...
  DevApp.prototype._renderCharts = function () {
    ...
    document.getElementById("chartSizeStyle").textContent = [
      ".sample {",
    ...
    if (settings.path) {
      ...
      specs = specs.filter(function (s) {
        return s.filePath.match(regex) || (s.description && s.description.match(regex));
      });
    }
    ...
    if (LAZY_RENDERING) {
      ...
      while (this._notRenderedSpecs.length) {
        ...
        chart = s.spec.type ? new Taucharts.Chart(s.spec) : new Taucharts.Plot(s.spec);
  ...
  DevApp.prototype._initUI = function () {
    ...
    var createCheckbox = function (name) {
      var node = createElement('<label><input type="checkbox" value="{{name}}"/>{{name}}</label>', { name: name });
      return {
        node: node,
    ...
    var onValueChanged = function (e) {
      var getValues = function (container) {
        return select(container, "input")
          .filter(function (el) {
            return el.value !== "all" && el.checked === true;
          })
          .map(function (el) {
            return el.value;
          });
    ...
    if (LAZY_RENDERING) {
      ...
      document.getElementById("samplesContainer").addEventListener("scroll", this._renderVisibleCharts.bind(this));
    }
  };
  ...
  function random(n) {
    ...
    var index = Math.min(Math.floor(Math.random() * items.length), items.length - 1);
    return items[index];
  }
